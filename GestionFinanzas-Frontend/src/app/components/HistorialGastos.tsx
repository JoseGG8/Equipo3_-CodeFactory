...
import { TrendingDown, Calendar, Search, SlidersHorizontal, Inbox } from 'lucide-react';
...
export function HistorialGastos() {
  ...
  }, [gastos, busqueda, orden, fechaDesde, fechaHasta]);

  const totalFiltrado = gastosFiltrados.reduce((s, t) => s + t.monto, 0);
  ...
  return (
    <div className="space-y-5">
      {/* Encabezado */}
      <div>
        <h1 className="text-2xl font-semibold text-gray-900">Historial de Gastos</h1>
        <p className="text-sm text-gray-500 mt-0.5">Visualiza y analiza todos tus gastos registrados</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 text-sm"
          />
        </div>
        <div className="relative">
          ...
          <select
            ...
            className="pl-9 pr-8 py-2.5 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 text-sm cursor-pointer appearance-none"
          >
      ...
      {gastosFiltrados.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
          ...
          <p className="text-gray-500 font-medium">Aún no hay gastos guardados para mostrar</p>
          <p className="text-sm text-gray-400 mt-1">
            {busqueda ? 'Intenta con otro término de búsqueda.' : 'Registra tu primer gasto para verlo aquí.'}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {gastosFiltrados.map((t) => (
            <div
              key={t.id}
              className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4 hover:border-red-200 transition-colors"
            >
              {/* Icono */}
              ...
              {/* Monto negativo */}
              <span className="text-red-600 font-semibold text-sm whitespace-nowrap flex-shrink-0">
                -{formatearMoneda(t.monto)}
              </span>
            </div>
          ))}
